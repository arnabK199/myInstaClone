            ...
            { data ? data.map(item => {
                return(
                    <div key = {item.title} className='card home-card'>
                        <h5 className='post-by'>
                        <img className='profile-mini' src={item.postedBy.pic} />
                        <Link className= 'user-profile' to = {item.postedBy._id == props.user._id ? '/profile' : `/user/${item.postedBy._id}`}>{item.postedBy.name} </Link>
                        ...
                        <div className='class-content'>
...

                            {
                                item.comments.map(comment => {
                                    ...
                                    {comment.commentedBy._id == props.user._id ? <i className="material-icons" style={{paddingTop:'5px'}} onClick={() => deleteComment(comment._id,item._id)}>delete</i> : ''}
